...
module.exports = {
  user(s) {
    ...
    if(!fs.existsSync(`./server/dbfile/user/${idb}`)) return {code:404};
    return {code:200,data:{name:`./server/dbfile/user/${idb}`}};
  },
  group(s) {
    const idb = db.ref.g[s]?.i;
    if(!idb) return {code:404};
    if(!fs.existsSync(`./server/dbfile/group/${idb}`)) return {code:404};
    return {code:200,data:{name:`./server/dbfile/group/${idb}`}};
  },
  content(uid, s) {
